...
const option_definitions = [
    ...
    { name: 'api-secret', alias: 's', type: String, description: 'The API secret to access the Nightscout instance if it has been locked down.'},
    { name: 'token', alias: 't', type: String, description: 'The access token of the Nightscout instance if it has been locked down.'},
...
function print_usage() {
    const usage = cli_usage([
        {
            header: 'Nightscout autotune',
            content: [
                'Run oref0-autotune against a Nightscout profile.',
                '',
                'Home: {underline https://github.com/houthacker/nightscout-autotune}'
            ]
        },
        {
            header: 'Options',
...
(async function run() {
    if(options['help'] !== undefined) {
        ...
        let ns_token = options['token'] || undefined;
        ...
        if (api_secret) {
            console.error(`passhprase:\t\t${api_secret}`);
        } else if (ns_token) {
            console.error(`token:\t\t\t${ns_token}`);
        }
        if (profile) {
